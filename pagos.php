<?php
  ...
  if($op=='Si'){                    //Elimina Registro
      ...
      $op='';
  		...
  		$lUp   = mysql_query("UPDATE cmc SET pagado=''");
  		
  		$PgsA  = mysql_query("SELECT medico,periodo FROM pgs WHERE movto='Pago'");
  		while($Pgs = mysql_fetch_array($PgsA)){
  		
  			$lUp = mysql_query("UPDATE cmc SET pagado='Si' WHERE medico='$Pgs[medico]' AND mes='$Pgs[periodo]'");
  		
  		}
  		
  		$Msj = "Listo!! tus pagos han sido actualizados en los movtos por pagar";
...
//  if(substr($busca,0,1)=='-'){
  if( $busca == ''){
     ...
     $cSql  = "SELECT $Qry[campos] FROM $Qry[froms] LEFT JOIN med ON pgs.medico=med.medico WHERE med.nombrec like '%$busca'";
  }elseif( $busca < 'a'){
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] LEFT JOIN med ON pgs.medico=med.medico WHERE pgs.idpgs='$busca'";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] LEFT JOIN med ON pgs.medico=med.medico WHERE $BusInt $Qry[filtro]";
  }   
  //echo $cSql;
  ...
  $aIzq   = array("Edita","-","-");	  //Arreglo donde se meten los encabezados; Izquierdos
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a href='$cLink?busca=$rg[idpgs]'><img src='lib/edit.png' alt='Modifica Registro' border='0'></td>";
    ...
    echo "<a class='pg' href='pagos.php?op=cs'>Consolida pagos en cxp</a>";

<?php
 ...
 if($filtro9<>'*'){
  ...
  if(substr($busca,0,1)=='-'){
     ...
     $cSql="SELECT $Qry[campos],med.fecmod FROM $Qry[froms] WHERE med.medico='$busca' $filtro2 $filtro4 $filtro6 $filtro8 $filtro10";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos],med.medico,med.fecmod FROM $Qry[froms] WHERE $BusInt $Qry[filtro] $filtro2 $filtro4 $filtro6 $filtro8 $filtro10";
     //echo $cSql;
  }   
  ...
  $aIzq   = array("Selecc","-","-","Ed","-","-","Imp","-","-");	  //Arreglo donde se meten los encabezados; Izquierdos
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a href=javascript:wingral('medicopdf.php?busca=$rg[medico]')><img src='images/printdigital.png' alt='Impresion de registro' border='0' width='22'></td>";
           ...
           if($rg[fecmod]=='0000-00-00'){
              $Actualiza=' ';
           }else{
              $Actualiza=substr($rg[fecmod], 0, 10);
           }       
...
echo "<form name='form10' method='post' action='$_SERVER[PHP_SELF]?pagina=0&Sort=Asc&orden=med.medico&busca=&filtro=*&filtro3=*&filtro5=*&filtro7=*&filtro9=*'>";
...
echo "<a  class='pg'  href='$_SERVER[PHP_SELF]?pagina=0&Sort=Asc&orden=med.medico&busca=&filtro=*&filtro3=*&filtro5=*&filtro7=*&filtro9=*'>Limpia pantalla</a></div>";

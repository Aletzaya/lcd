<?php
 ...
 if($filtro3=='*'){
  ...
  $Titulo = "Demanda de afluencia de pacientes por institucion del $FechaI al $FechaF";
	...
	WHERE ot.institucion=inst.institucion $filtro4 and ot.fecha Between '$FechaI' And '$FechaF'
...
<body bgcolor="#FFFFFF">
    ...
    while ($reg = mysql_fetch_array($OtA)) {
		...
		if( ($nRng % 2) > 0 ){$Fdo='FFFFFF';}else{$Fdo=$Gfdogrid;}    //El resto de la division;
			...
			</td><td width='22%'>$Gfont $reg[nombre] </font></td><td aling='rigth' width='5%'>$Gfont <font size='1'>$reg[status]</font></td><td align='center' width='15%'>$Gfont $reg[condiciones]</font></td><td>$Gfont $reg[promotorasig]</font></td>";
			echo $cTit . "<td align='right'>$Gfont ".number_format($reg[importe],'2')."</td><td align='center'>$Gfont ".number_format($reg[3],'0')." </td>
			<td align='right'>$Gfont ".number_format($reg[importe],'2')."</font>
			<td align='center'><a href=javascript:winuni('visitasmed.php?FechaI=$Ini&FechaF=$Fin&Med=$Med')>$Gfont $reg[fecha]</a></font></td></tr>";
...
//					onMouseOut=this.style.backgroundColor='$vist';>$Gfon ".number_format($reg[importe],'2')."</font></td>";
...
//				WHERE pgs.medico='$Inst' and date_format(pgs.fecha,'%Y%m') Between '$Ini' And '$Fin'";
//				
//				$OtB  = mysql_fetch_array(mysql_query($cOtB,$link));
//
//                echo "<tr bgcolor='$Fdo' onMouseOver=this.style.backgroundColor='$Gbarra';this.style.cursor='hand' onMouseOut=this.style.backgroundColor='$Fdo';>
//				<td align='center'><a href=javascript:winuni('comisionesmed.php?FecI=$FechaI&FecF=$FechaF&Institucion=*&$Sucursal=1&Promotor=$filtro5&Medico=$Inst')>$Gfont $Inst</a>
//				</td><td width='22%'>$Gfont $Nombre </font></td><td aling='rigth' width='5%'>$Gfont <font size='1'>$Status</font></td><td align='center' width='15%'>$Gfont $Condiciones</font></td><td>$Gfont $Promotor</font></td>";
//                echo $cTit . "<td align='center'>$Gfon ".number_format($SubT,'0')."</td><td align='center'>$Gfon ".number_format($Venta,'2')." </td>
//				<td align='right'>$Gfont ".number_format($Promedio,'2')."</font>
...
//        }
//		$cOtC= "select medico,count(*),date_format(pgs.fecha,'%Y%m') from pgs
//		WHERE pgs.medico='$Inst' and date_format(pgs.fecha,'%Y%m')='$i'";
//		
//		$OtC  = mysql_fetch_array(mysql_query($cOtC,$link));
//		
//		if($OtC[1]==0){
...
//		$cTit = $cTit . "<td align='center' bgcolor='$vist' 
//		onMouseOver=this.style.backgroundColor='$Gbarra';this.style.cursor='hand'
//		onMouseOut=this.style.backgroundColor='$vist';>$Gfon ".number_format($importem,'2')."</font></td>";
//        $SubT+=$aCnt[$x];
//        $GraT+=$aCnt[$x];
//		$tCnt[$x] = $tCnt[$x] + $aCnt[$x];
//    }
...
//	WHERE pgs.medico='$Inst' and pgs.fecha Between '$FechaI' And '$FechaF'";
//
//	if( ($nRng % 2) > 2 ){$Fdo='FFFFFF';}else{$Fdo=$Gfdogrid;}    //El resto de la division;
//	$OtB  = mysql_fetch_array(mysql_query($cOtB,$link));
//	echo "<tr bgcolor='$Fdo' onMouseOver=this.style.backgroundColor='$Gbarra';this.style.cursor='hand' onMouseOut=this.style.backgroundColor='$Fdo';>
//	<td align='center'><a href=javascript:winuni('comisionesmed.php?FecI=$FechaI&FecF=$FechaF&Institucion=*&$Sucursal=1&Promotor=$filtro5&Medico=$Inst')>$Gfont $Inst</a></td>
//	<td>$Gfont $Nombre</font></td><td aling='rigth'>$Gfont <font size='1'>$Status</font></td><td align='center'>$Gfont $Condiciones</font></td><td>$Gfont $Promotor</font></td>";
//	echo $cTit . "<td align='center'>$Gfon ".number_format($SubT,'0')."</td><td align='center'>$Gfon ".number_format($Venta,'2')." </td>
//	<td align='right'>$Gfont ".number_format($Promedio,'2')."</font>
//	<td align='center'><a href=javascript:winuni('visitasmed.php?FechaI=$Ini&FechaF=$Fin&Med=$Med')>$Gfon $OtB[1]</a></font></td></tr>";
//	$Tvisit+=$OtB[1];

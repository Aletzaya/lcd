<?php
  ...
  if(!isset($Msj)){
    $Msj = "Para buscar por orden poner un guion al principio(-)";  
  ...
  if($Palabras > 1){
     ...
     for ($i = 0; $i < $Palabras; $i++) {
            if(!isset($BusInt)){$BusInt=" cja.usuario like '%$P[$i]%' ";}else{$BusInt=$BusInt." and cja.usuario like '%$P[$i]%' ";}
     }
  }else{
     $BusInt=" cja.id >= '$busca' ";  
  }
  ...
  if(substr($busca,0,1)=='-'){
     ...
     $cSql  = "SELECT $Qry[campos],ot.institucion,ot.orden FROM $Qry[froms],ot WHERE cja.orden='$busca' and cja.orden=ot.orden";
  
     
  }else{

      #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos],ot.institucion,ot.orden FROM $Qry[froms],ot WHERE $BusInt $Qry[filtro] and cja.orden=ot.orden";
//     $cSql="SELECT $Qry[campos],ot.institucion,ot.orden FROM $Qry[froms],ot WHERE $cQry $BusInt $Qry[filtro] and cja.orden=ot.orden";
  ...
  $aIzq   = array("Recib_Inv","-","-","Recib_Gral","-","-");	  //Arreglo donde se meten los encabezados; Izquierdos
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a class='vt' href=javascript:Ventana('impotpdf.php?busca=$rg[orden]&pagina=$pagina&reimp=1&ingreso=$rg[id]')><img src='lib/print.png' alt='Edita registro' border='0'></a></td>";
           echo "<td align='center'><a class='vt' href=javascript:Ventana('impotpdf-ant.php?busca=$rg[orden]&pagina=$pagina&reimp=1&ingreso=$rg[id]')><img src='lib/print.png' alt='Edita registro' border='0'></a></td>";

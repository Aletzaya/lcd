...
<body bgcolor="#FFFFFF">
    ...
    while ($reg = mysql_fetch_array($OtA)) {
		...
		</td><td width='22%'>$Gfont $reg[nombre] </font></td><td aling='right' width='5%'>$Gfont <font size='1'>$reg[status]</font></td><td align='center' width='15%'>$Gfont $reg[condiciones]</font></td><td>$Gfont $reg[promotorasig]</font></td>";
		echo $sTit . "<td align='center'>$Gfont ".number_format($reg[3],'0')." </td>
		<td align='right'>$Gfont ".number_format($reg[importe],'2')."</font>
		<td align='right'>$Gfont ".number_format($importemt/$contart,'2')."</font>
		<td align='center'><a href=javascript:winuni('visitasinst.php?FechaI=$Ini&FechaF=$Fin&Inst=$reg[institucion]')>$Gfont $reg[fecha2]</a></font></td></tr>";

<?php
  ...
  if( $busca < 'a'){
     $cSql="SELECT $Qry[campos],ot.ubicacion,ot.encaja,ot.status FROM cli,$Qry[froms] WHERE ot.cliente=cli.cliente AND ot.orden >= '$busca' $Qry[filtro]";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos],ot.ubicacion,ot.encaja,ot.status FROM cli,$Qry[froms] WHERE ot.cliente=cli.cliente AND $BusInt $Qry[filtro]";
  }   
  //echo $cSql;
           ...
           //echo "<td align='center'><img src='lib/deleoff.png' alt='Elimina Registro' border='0'></td>";
			if($rg[encaja]=='Si' OR $rg[status]=='Entregada'){ 
              ...
              //echo "<td>$Gfont $rg[encaja]</font></td>";
				...
				//echo "<td align='center'><a class='pg' href=javascript:winuni('ordenrec.php?Recibeencaja=$Usr&Orden=$rg[orden]&boton=Enviar')><img src='lib/browse.png' border='0'></a></td>";

  ...
  if($Depto<>"*"){
  		if($Subdepto<>"*"){
				 if($Suc<>"*"){
					...
					AND otd.orden>='$busca' and ot.fecha>='2015-01-01' and est.subdepto='$Subdepto' and ot.suc='$Suc'";
					...
					FROM ot,est,otd,cli
					WHERE ot.orden=otd.orden AND ot.cliente=cli.cliente AND otd.estudio=est.estudio AND est.depto='$Depto' 
					AND otd.orden>='$busca' and ot.fecha>='2015-01-01' and est.subdepto='$Subdepto'";
				 }
		}else{
			if($Suc<>"*"){
				$SqlA="SELECT cli.nombrec,ot.orden,ot.fecha,ot.hora,otd.estudio,est.descripcion,
				...
				FROM ot,est,otd,cli
				WHERE ot.orden=otd.orden AND ot.cliente=cli.cliente AND otd.estudio=est.estudio AND est.depto='$Depto' 
				AND otd.orden>='$busca' and ot.fecha>='2015-01-01' and ot.suc='$Suc'";
			}else{
				$SqlA="SELECT cli.nombrec,ot.orden,ot.fecha,ot.hora,otd.estudio,est.descripcion,
				otd.status,otd.etiquetas,est.muestras,ot.institucion,otd.capturo,otd.recibeencaja,otd.cuatro,
				otd.recibeencaja,est.depto,ot.suc,otd.obsest,ot.observaciones,otd.alterno
				FROM ot,est,otd,cli
				WHERE ot.orden=otd.orden AND ot.cliente=cli.cliente AND otd.estudio=est.estudio AND est.depto='$Depto' 
				AND otd.orden>='$busca' and ot.fecha>='2015-01-01'";
			}
		}
  }else{
	 if($Suc<>"*"){
			...
			otd.recibeencaja,est.depto,ot.suc,otd.obsest,ot.observaciones,otd.alterno
            FROM ot,est,otd,cli
            WHERE ot.orden=otd.orden AND ot.cliente=cli.cliente AND otd.estudio=est.estudio AND otd.orden>='$busca' and ot.fecha>='2015-01-01'";
	 }
  }

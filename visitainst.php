<?php
  ...
  #Variables comunes;
  $Titulo = "Registro de Visita Inst";
  $op     = $_REQUEST[op];
  $Msj    = $_REQUEST[Msj];
  $Id     = 46; 
...
//  if(substr($busca,0,1)=='-'){
  if( $busca == ''){
     ...
     $cSql  = "SELECT $Qry[campos] FROM $Qry[froms] LEFT JOIN inst ON vinst.inst=inst.institucion WHERE inst.nombre like '%$busca'";
  }elseif( $busca < 'a'){
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] LEFT JOIN inst ON vinst.inst=inst.institucion WHERE vinst.id='$busca'";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] LEFT JOIN inst ON vinst.inst=inst.institucion WHERE $BusInt $Qry[filtro]";
  }   
  //echo $cSql;

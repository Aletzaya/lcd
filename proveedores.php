<?php
  ...
  if($op=='Si'){                    //Elimina Registro
      $EmpA=mysql_query("SELECT proveedor FROM ot WHERE cliente='$_REQUEST[cId]' limit 1",$link);
      $Emp=mysql_fetch_array($EmpA);
      if ($Emp[cliente]<>""){
 			$Msj="No es posible eliminar este empleado pues hay checadas registradas";
    	}else{			  
          $lUp=mysql_query("DELETE FROM prv WHERE id='$_REQUEST[cId]' limit 1",$link);
  ...
  if($busca >= 'a'){
     ...
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE prv.id >='$busca' $BusInt  $Qry[filtro]";
  ...
  $aIzq   = array("Edit","-","-","Imp","-","-");				    //Arreglo donde se meten los encabezados; Izquierdos
 ...
 headymenu($Titulo,1);
  ...
  if(!$res=mysql_query($cSql,$link)){
      ...
      while($registro=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a href=javascript:wingral('proveedorespdf.php?busca=$registro[id]')><img src='images/printdigital.png' alt='Impresion de registro' border='0' width='22'></td>";

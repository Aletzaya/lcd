...
<?php
    ...
    $cSql="SELECT otd.usrest,date_format(ot.fecha,'%Y-%m') as fecha,count(*),ot.orden,ot.suc,ot.institucion,otd.estudio,est.depto,est.subdepto,count(distinct ot.orden),sum(otd.precio) as precios,sum(otd.precio*(otd.descuento/100)) as descuentos
     from otd,ot,est
    	WHERE otd.estudio = est.estudio and otd.usrest='$Capt' and ot.orden=otd.orden and ot.fecha Between '$FechaI' And '$FechaF' $filtro4 $filtro6 $filtro8

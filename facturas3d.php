<?php
  ...
  if ($_REQUEST[Boton] == 'Agregar') {
    ...
    $Valida = mysql_query("SELECT factura FROM ot WHERE orden='$_REQUEST[Orden]'");
    $Validado = mysql_fetch_array($Valida);

    if ($Validado[factura] == '0') {

        $result = mysql_query("SELECT * FROM otd WHERE orden='$_REQUEST[Orden]'");
        while ($rg = mysql_fetch_array($result)) {

            $Precio = $rg[precio] * (1 - ($rg[descuento] / 100));
            $PrecioU = $Precio / (1 + ($Cia[iva] / 100));

            $lUp = mysql_query("INSERT INTO fcd (id,estudio,precio,descuento,orden,iva,cantidad)
                  VALUES ('$busca','$rg[estudio]','$PrecioU','0','$_REQUEST[Orden]','$Cia[iva]','1')");
        }
   ...
   echo "</table>";
    ...
    echo "<tr><td height='280' align='center' valign='top' class='texto_tablas'>";
           ...
           //============Cierra de tabla 4==============
                ...
                echo " &nbsp; <a class='pg' href='facturas3.php?Sort=Asc&orden=fc.id&busca='><img src='lib/regresa.jpg' border=0></a> ";
                ...
                if($He[status] == 'Abierta'){                
                   ...
                   echo " &nbsp; &nbsp; &nbsp; $Msj";
                        ...
                        if ($He[importe] > 0) {
                                echo "<a  class='Cuadro_rojo' href='genfactura3.php?ord=$orden'>&nbsp;<b>CLICK AQUI PARA GENERAR LA FACTURA&nbsp; <input type='hidden' name='Ord' value='$registro[orden]'></a>";                    
...
function Totaliza($busca){	//busca es idnvo de medt y cVarVal es id de la entrada;
     ...
     $DddA  = mysql_query("SELECT sum((precio*(1-descuento/100))*cantidad) as importe,
              sum(cantidad) as cantidad 
              FROM fcd WHERE id='$busca' and cantidad > 0");         
         
     $Ddd   = mysql_fetch_array($DddA);

     $DescA  = mysql_query("SELECT sum(precio*cantidad) as descuento
               FROM fcd WHERE id='$busca' and cantidad < 0");         
         
     $Desc  = mysql_fetch_array($DescA);
     ...
     if($Ddd[0]==0){
        ...
        $Cnt        = $Ddd[cantidad];
        //$Total   = $Ddd[importe];
        $Importe = round($Ddd[importe]+$Desc[descuento],2);
        $Iva        =  round($Importe * ($Cia[iva]/100),2);
        $Total     = $Importe + $Iva;

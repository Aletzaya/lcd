<?php
  ...
  if($op=="pr"){			//Cuando entras p agregar el encabezado
     ...
     $Fecha  = date("Y-m-d");
     $InsA   = mysql_query("SELECT lista,descuento FROM inst WHERE institucion='$_REQUEST[Institucion]'",$link);    #Checo la lista de la institucion
    ...
    $OtnvaA   = mysql_query("SELECT lista,fechae,servicio FROM otnvas WHERE usr='$Usr' and venta='$Vta'");
    if($Otnva = mysql_fetch_array($OtnvaA)){
        ...
        if($Otnva[lista] <> 0){
           ...
           $EstA   = mysql_query("SELECT estudio,descripcion,$Lista,entord,entexp,enthos,enturg
                     FROM est
           ...
           if($cCpo[estudio] <> ''){
               ...
               if( $cCpo[$Lista] > 0){
                    ...
                    $lUp     = mysql_query("INSERT INTO otdnvas (usr,estudio,descripcion,descuento,precio,venta)
                               VALUES
                               ('$Usr','$Estudio','$cCpo[1]',0,'$cCpo[$Lista]','$Vta')");
     ...
     $InsA   = mysql_query("SELECT lista,descuento FROM inst WHERE institucion='1'",$link);    #Checo la lista de la institucion
     $Ins    = mysql_fetch_array($InsA);
    ...
    $lUp = mysql_query("INSERT INTO ot
          ...
          receta,importe,descuento,pagada,fecpago,medicon,status,horae,suc,folio)
          VALUES
          ...
          '$Ot[descuento]','$cPag','$Fecha','$_REQUEST[Medicon]','DEPTO','$Ot[horae]','$Suc','$Folio')");
...
$InsA = mysql_query("SELECT institucion,nombre,alias,descuento FROM inst order by institucion");

$MedA = mysql_query("SELECT nombrec FROM med WHERE medico='$Ot[medico]'");
$Med  = mysql_fetch_array($MedA);

$CliA = mysql_query("SELECT nombrec,programa,observaciones,colonia,municipio,numveces FROM cli WHERE cliente='$Ot[cliente]'");
...
echo "<a class='vt' href='ordenesnvas.php?Vta=$Vta&Usr=$Usr&op=br'>[Borra/todo]</a> &nbsp; ";
...
echo "</tr></table>";
  ...
  echo "<tr>";
    echo "<td width='26%' valign='top'>";
        ...
        echo "<form name='form1' method='post' action='ordenesnvas3.php?op=pr'>";
        ...
        echo "<input type='hidden' name='Vta' value=$Vta><br> ";
  ...
  echo "<tr>";
    echo "<td valign='top'>";
     echo "<form name='form2' method='post' action='ordenesnvas3.php?op=Estudio&Vta=$Vta'> ";
         echo "$Gfont <strong>ESTUDIO</strong>: ";
     ...
     echo " &nbsp; <b><a class='pg' href='ordenes.php?orden=ot.orden'>REGRESAR</a></b> ";

   echo "</td>";
    echo "<td valig='top'>";
        echo "<form name='form3' method='post' action='ordenesnvas3.php?op=cl&Vta=$Vta'>";
            $nPrg=$Cli[programa];
            echo "$Gfont <strong>PACIENTE</strong> : ";
            ...
            echo " &nbsp; <a class='vt' href=javascript:winuni('Upcliente.php?busca=$Ot[cliente]') title='Click para actualizar sus datos'>".substr($Cli[nombrec],0,35)."</a><br>";
            echo " &nbsp; Programa: $aPrg[$nPrg] &nbsp; &nbsp; ";
            
            if($Cli[observaciones]<>''){
               echo "<a href=javascript:winuni('cliobs.php?busca=$Ot[cliente]')><img src='lib/int4.gif' border='0' height='35'></a>";
            }
            
            echo " Col.".ucwords(strtolower($Cli[colonia])) . " " . ucwords(strtolower($Cli[municipio])) . ' vecs: <font color=#FF0000 size=+3>'. $Cli[numveces];
        echo "</form>";

    echo "</td>";
    echo "<td valign='top'>";
      echo "<form name='form4' method='post' action='ordenesnvas3.php?Vta=$Vta&op=md'>";
         echo "$Gfont <strong>MEDICO: </strong>";
         echo "<input class='textos' class='textos' name='Medico' type='text' value='$Ot[medico]' size='5'> ";
         echo "<a href='medicos.php?orden=med.medico'><img src='lib/lupa_o.gif' alt='Catalogo de medicos' border='0'></a>";
         echo substr($Med[nombrec],0,45);
      echo "</form>";
    echo "</td>";
  echo "</tr>";
...
echo "</table>";
  ...
  if(!$res=mysql_query($cSql.$cWhe,$link)){
        ...
        while($registro=mysql_fetch_array($res)){
            ...
            if ($Ins[descuento]==0){
				$Desc=$registro[descuento];
			}else{
				$Desc=$Ins[descuento];
			}
            echo "<td align='center'>$Gfont <a class='pg' href='descuentos.php?Vta=$Vta&op=dt&Estudio=$registro[estudio]'>$Desc</a></td>";
			echo "<td align='right'>$Gfont ".number_format($registro[precio]*(1-$Desc/100),"2")."</td>";
            echo "<td align='center'>$Gfont <a href='ordenesnvas3.php?op=Eli&busca=$registro[estudio]&Vta=$Vta'><img src='lib/dele.png' alt='Elimina' border='0'></a></td>";
            echo "</tr>";
            $nImp=$nImp+($registro[precio]*(1-($Desc/100)));
            $nRng++;
        }

<?php
  ...
  if($_REQUEST[op]=='El' AND $Usr=='nazario'){
...
//						} 
   ...
   <td>$Gfont <font color='#ffffff'>Fecha/Hora: $He[fecha]&nbsp; $He[hora]&nbsp;Fecha/entrega: $He[fechae]&nbsp; $He[horae] </td><td>$Gfont <font color='#ffffff'> Pagada: $He[pagada]</td>";
...
echo "<table align='center' width='100%' border='0' cellspacing='1' cellpadding='0'>";
echo "<tr><td align='center'><form name='form1' method='post' action='ordenesd5.php'>";
...
// ++++++++++++++++++++ Tabla de insumos +++++++++++++++++++
  ...
  $cSqlE     = "SELECT otd.estudio,est.descripcion,estd.estudio,estd.producto,estd.cantidad,invl.descripcion,est.estpropio,invl.clave
   ...
   if(!$res2=mysql_query($cSqlE." ORDER BY ".$OrdenDef)){
    ...
    echo "<form name='form5' method='get' action='ordenesd.php'>";
		...
		while($registro2=mysql_fetch_array($res2)){
			...
			if(!$resF=mysql_query($cSqlF." ORDER BY ".$OrdenDef)){
				...
				$unidad=$sucursal;
				$pcs="<td align='center'>$Gfont<a class='pg' href='ordenesd.php?busca=$busca&pagina=$pagina&op=pcs&Estudio=$registro2[estudio]&insumo=$registro2[clave]&cnt=$cantidad3&intext=$intext&unidad=$Maquila&pcs=1'>OK</b></a></td>";
	
			}else{
				$registro3=mysql_fetch_array($res);
			...
			if($intext=='Interno'){
				...
				<a class='pg' href='ordenesd.php?busca=$busca&pagina=$pagina&op=pcs&Estudio=$registro2[estudio]&insumo=$registro2[clave]&cnt=$cantidad3&intext=Externo&unidad=$Maquila'> Externo / </a>
				<a class='pg' href='ordenesd.php?busca=$busca&pagina=$pagina&op=pcs&Estudio=$registro2[estudio]&insumo=$registro2[clave]&cnt=$cantidad3&intext=Mixto&unidad=$Maquila'> Mixto </b></a></td>";
			}elseif($intext=='Externo'){
				echo "<td align='center'>$Gfont <a class='pg' href='ordenesd.php?busca=$busca&pagina=$pagina&op=pcs&Estudio=$registro2[estudio]&insumo=$registro2[clave]&cnt=$cantidad3&intext=Interno&unidad=$Maquila'>Interno / </a>
			...
			if($unidad=='1'){
				...
				echo "<td align='center'>$Gfont <a class='pg' href='ordenesd.php?busca=$busca&pagina=$pagina&op=pcs&Estudio=$registro2[estudio]&insumo=$registro2[clave]&cnt=$cantidad3&intext=$intext&unidad=1'>LCD / 

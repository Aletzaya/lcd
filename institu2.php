<?php
 ...
 if($filtro3=='*'){
  ...
  if( $busca < 'a'){ //Busco por numero de inst.
      $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE inst.institucion >= '$busca' $Qry[filtro] $filtro2 $filtro4";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE $BusInt $Qry[filtro] $filtro2 $filtro4";
  }   
  //echo $cSql;
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      $res  = mysql_query($sql);
    	  ...
    	  echo "<form name='form' method='post' action='institu2.php?pagina=$pagina&Sort=Asc&busca=$busca&filtro=$filtro&filtro3=$filtro3'>";

		...
		if(strlen($Departamento)>0){
			if(strlen($Institucion)>0){
				if($Externo=="S"){
					...
					otd.etiquetas,otd.obsest,ot.suc,otd.status as statusotd,otd.recibeencaja,otd.recibepac
			...
			if(strlen($Institucion)>0){
				if($Externo=="S"){
					...
					otd.obsest,ot.suc,otd.status as statusotd,otd.recibeencaja,otd.recibepac
					FROM ot, cli, otd, est
					WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio and ot.fecha>='$Fechai' and
					...
					otd.status as statusotd,otd.recibeencaja,otd.recibepac
					FROM ot, cli, otd, est
					WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio and ot.fecha>='$Fechai' and
		...
		if(strlen($Departamento)>0){
			...
			if(strlen($Institucion)>0){
				if($Externo=="S"){
					...
					ot.suc,otd.status as statusotd,otd.recibeencaja,otd.recibepac
					FROM ot, cli, otd, est
					WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio and ot.fecha>='$Fechai' and
		...
		if(strlen($Departamento)>0){
			if(strlen($Institucion)>0){
				if($Externo=="S"){
					...
					ot.hora, ot.servicio, ot.fechae, otd.lugar, otd.uno, otd.dos, otd.tres, otd.cuatro, otd.cinco, otd.seis, 
					ot.status, est.depto, dep.departamento, ot.horae, est.estpropio, otd.statustom, otd.usrest, otd.fechaest, ot.observaciones,
					otd.etiquetas,otd.obsest,otd.status as statusotd,otd.recibeencaja,otd.recibepac
					FROM ot, cli, otd, est, dep
					WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio and ot.fecha>='$Fechai' and
					ot.fecha <='$Fechaf' and ot.status<>'Entregada' and ot.institucion='$Institucion' and dep.departamento=$Departamento 
...
//    echo "<th align='CENTER'><font size='1' face='Arial, Helvetica, sans-serif'>R. Est.</font></th>";		
    ...
    echo "<th align='CENTER'><font size='1' face='Arial, Helvetica, sans-serif'>R.Pac.</font></th>";
    echo "<th align='CENTER'><font size='1' face='Arial, Helvetica, sans-serif'>Recibe Pac.</font></th>";
    echo "<tr><td colspan='9'><hr noshade></td></tr>";
    $Orden=0;
    $Ordenes=1;
	$Estudios=0;
    while($rg=mysql_fetch_array($UpA)) {
    	if($Orden<>$rg[orden]){
    		...
    		echo "<th colspan='2' align='left'><font size='1' face='Arial, Helvetica, sans-serif'>Fecha Cap.: $rg[fecha] &nbsp;&nbsp;&nbsp;&nbsp; $rg[hora] &nbsp;&nbsp;&nbsp;&nbsp; $Rec &nbsp;&nbsp;&nbsp;&nbsp; $Gfont4 $Urgencia &nbsp;&nbsp;&nbsp;&nbsp; Fech/hra entega: $rg[fechae] &nbsp;&nbsp; $rg[horae]</font></th>";
    		echo "</tr>";
    	    $Orden=$rg[orden];
    	}
		...
		echo "<th align='right' $Gfont9>$Gfont8 $stotd &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font>
		<font size='1' face='Arial, Helvetica, sans-serif'>$rg[estudio]&nbsp;-&nbsp;</font></th>";
		echo "<th align='left' $Gfont9><font size='1' face='Arial, Helvetica, sans-serif'> $rg[descripcion] &nbsp;&nbsp;&nbsp; $Gfont3 $pendiente &nbsp;&nbsp;&nbsp; $Gfont5 $Urgencia2</font></th>";
//		echo "<th align='center' $Gfont9><font size='1' face='Arial, Helvetica, sans-serif'>$Gfont7 $fechatn &nbsp; $horatn &nbsp;&nbsp; $rg[usrest] &nbsp;&nbsp;&nbsp; $Gfont6 $rg[statustom] &nbsp;&nbsp;&nbsp; ";
...
//		echo "<th align='center'><font size='1' face='Arial, Helvetica, sans-serif'><img src=images/$imagen2 alt=$dos></font></th>";
		...
		echo "<th align='center' $Gfont9><font size='1' face='Arial, Helvetica, sans-serif'>$Gfont8 $rg[recibeencaja] </font></th>";
		echo "<th align='center'><font size='1' face='Arial, Helvetica, sans-serif'><img src=images/$imagen6></font></th>";
		echo "<th align='center' $Gfont9><font size='1' face='Arial, Helvetica, sans-serif'>$Gfont8 $rg[recibepac] </font></th>";
   		echo "</tr>"; 
		if($rg[obsest]<>''){
		echo "<tr>";
	 ...
	 echo "<p align='center'><font face='verdana' size='-2'><a href='pidedatos.php?cRep=26&fechas=1&FecI=$FecI&FecF=$FecF'>";
...
<?php
	...
	echo "<form name='form1' method='post' action='pidedatos.php?cRep=26&fechas=1&FecI=$FecI&FecF=$FecF'>";

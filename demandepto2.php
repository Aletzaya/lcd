...
if ($Sucursal <> '*') {
	...
	if(strlen($Departamento)>0){
		if(strlen($Institucion)>0){
			...
			$cSql="SELECT otd.estudio,est.descripcion,otd.precio,count(otd.orden),sum(otd.precio),
			sum(otd.precio * (otd.descuento/100)),count(distinct ot.orden),est.depto,est.subdepto,otd.orden,dep.departamento
			FROM otd, est, ot, dep
			WHERE otd.estudio = est.estudio and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.orden=otd.orden and 
			ot.institucion=$Institucion and dep.departamento=$Departamento and est.depto=$Departamento and ($Sucursal)
			GROUP BY est.depto, est.subdepto, otd.estudio";
	
			$registro3=mysql_query("select count(distinct ot.orden) from ot,dep,est,otd where otd.estudio=est.estudio 
			and ot.orden=otd.orden and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.institucion=$Institucion 
			and est.depto=$Departamento and dep.departamento=$Departamento and ($Sucursal)");
			...
			and dep.departamento=$Departamento and est.depto=$Departamento and ($Sucursal)
			GROUP BY est.depto, est.subdepto, otd.estudio";
	
			$registro3=mysql_query("select count(distinct ot.orden) from ot,dep,est,otd where otd.estudio=est.estudio 
			and ot.orden=otd.orden and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and est.depto=$Departamento and 
			dep.departamento=$Departamento and ($Sucursal)");
			$registro2=mysql_fetch_array($registro3);
		}
	}else{
		if(strlen($Institucion)>0){
			...
			$Titulo="Demanda de estudios X Depto. Detallado del $Fechai al $Fechaf Institucion : $Institucion $Nombre[0]";
			$cSql="SELECT otd.estudio, est.descripcion, otd.precio, count(otd.orden), sum(otd.precio),
			sum(otd.precio * (otd.descuento/100)), count(distinct ot.orden), est.depto, est.subdepto, otd.orden
			FROM otd, est, ot
			WHERE otd.estudio = est.estudio and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.orden=otd.orden 
			and ot.institucion=$Institucion and ($Sucursal)
			GROUP BY est.depto, est.subdepto, otd.estudio";
			...
			if ($Sucursal <> '*') {
				if(strlen($Departamento)>0){
					if(strlen($Institucion)>0){
						...
						FROM otd, est, ot, dep
						WHERE otd.estudio = est.estudio and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.orden=otd.orden and dep.departamento=$Departamento and est.depto=$Departamento
						and ot.institucion=$Institucion and est.depto='$rg[0]' and ($Sucursal) group by est.depto";
					}else{
						$cSql="SELECT count(otd.orden),sum(otd.precio),sum(otd.precio * (otd.descuento/100)),count(distinct otd.orden)
						FROM otd, est, ot, dep
						WHERE otd.estudio = est.estudio and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.orden=otd.orden and dep.departamento=$Departamento and est.depto=$Departamento
						and est.depto='$rg[0]' and ($Sucursal) group by est.depto";
					}
				}else{
					if(strlen($Institucion)>0){
						$cSql="SELECT count(otd.orden),sum(otd.precio),sum(otd.precio * (otd.descuento/100)),count(distinct otd.orden)
...
echo "<p align='center'><font face='verdana' size='-2'><a href='pidedatos.php?cRep=10&fechas=1&FecI=$FecI&FecF=$FecF'>";
echo "Regresar</a></font>";
echo "</div>";
?>
</font>
<div align="left">
<form name="form1" method="post" action="pidedatos.php?cRep=10&fechas=1&FecI=$FecI&FecF=$FecF">

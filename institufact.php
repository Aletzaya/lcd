<?php
  ...
  #Variables comunes;
  $Titulo = "Facturacion de Instituciones";
  $op     = $_REQUEST[op];
  $Msj    = $_REQUEST[Msj];
  $Id     = 59; 
 ...
 if($filtro=='*'){
	...
	$filtro4="and inst.status='ACTIVO'";
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      $res  = mysql_query($sql);
    	  ...
    	  echo "<form name='form' method='post' action='institufact.php?pagina=$pagina&Sort=Asc&busca=$busca&filtro=$filtro&filtro3=$filtro3&filtro5=$filtro5'>";
...
/*		  echo "<td align='right'>$Gfont<b><font size='1' color='#009900'>Status</b></font>";
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a href=institufacte.php?busca=$rg[institucion]&pagina=$pagina&filtro=*&filtro3=*><img src='lib/cuentas.png' alt='Edita registro' border='0'></a></td>";

           Display($aCps,$aDat,$rg);    
/*
          $PenA = mysql_query("SELECT count(status) as pend FROM instfact WHERE instfact.inst='$rg[institucion]' and instfact.status='Pendiente'");

          $Pen=mysql_fetch_array($PenA);

          if($Pen[pend]>=1{

            echo "<td align='center' colspan='2'>$Pen[pend] Pendientes</td>";
...
*/
    ...
    PonPaginacionSA(true);           #-------------------pon los No.de paginas-------------------

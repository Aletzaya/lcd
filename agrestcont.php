<?php
  ...
  if(isset($_REQUEST[buscar]))  { $_SESSION['buscar'] = $_REQUEST[buscar];}     #Campo por el cual se ordena 
  ...
  $buscar    = $_REQUEST[buscar];        //Dato a busca
  $estudio    = $_REQUEST[estudio];        //Dato a busca
  #Variables comunes;
  ...
  if(substr($busca,0,1)=='-'){
     ...
     $cSql  = "SELECT $Qry[campos],activo FROM $Qry[froms] WHERE est.estudio='$busca' and est.base='Individual'";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos],activo FROM $Qry[froms] WHERE $BusInt $Qry[filtro] and est.base='Individual'";
  }   
...
while ($registro = mysql_fetch_array($res)) {
    ...
    echo "<td>$Gfont <a class='pg' href='estudiose2.php?op=Reg_Estudio&busca=$estudio&opm=contenido&Estudio=$registro[estudio]&descripcion=$registro[descripcion]'>Seleccionar</a></td>";
    ...
    echo "<td>$Gfont $registro[depto]</td>";
    echo "<td>$Gfont $registro[subdepto]</td>";
    echo "<td>$Gfont $registro[bloqbas]</td>";
    echo "<td>$Gfont $registro[bloqeqp]</td>";
    echo "<td>$Gfont $registro[bloqmue]</td>";
    echo "<td>$Gfont $registro[bloqcon]</td>";
    echo "<td>$Gfont $registro[bloqdes]</td>";
    echo "<td>$Gfont $registro[bloqadm]</td>";
    echo "<td>$Gfont $registro[bloqatn]</td>";
...
CuadroInferior5($busca);    #-------------------Op.para hacer filtros-------------------


echo " <a href='estudiose2.php?busca=$estudio&opm=contenido'><img src='lib/regresar.gif' border=0></a> ";

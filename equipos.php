<?php
  ...
  #Variables comunes;
  $Msj     = $_REQUEST[Msj];
  $Titulo = "Catalogo de equipos";
  $op     = $_REQUEST[op];
  $Id     = 51; 										                 //Numero de query dentro de la base de datos
  
  #Intruccion a realizar si es que mandan algun proceso
  if($op=='Si'){                    //Elimina Registro
      $lUp=mysql_query("DELETE FROM eqp WHERE id='$_REQUEST[cId]' limit 1",$link);
  ...
  if($busca >= 'a'){
     ...
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE eqp.id >='$busca' $BusInt  $Qry[filtro]";

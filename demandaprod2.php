...
*/
		...
		$cSql="SELECT otd.estudio, count(otd.orden), conest.descripcion
		FROM otd, ot, conest
		WHERE  conest.estudio=otd.estudio  and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.orden=otd.orden
        GROUP BY otd.estudio";

     /*
        $cSql="SELECT  otd.estudio as est, conest.estudio, conest.descripcion
    FROM otd, ot, conest
    WHERE conest.estudio=otd.estudio and ot.fecha>='$Fechai' and ot.fecha<='$Fechaf' and ot.orden=otd.orden
    GROUP BY conest.estudio";
    */
	}
...
/*
    ...
    while($regest=mysql_fetch_array($UpAEst)) {
      ...
      echo "<td><font size='1' face='Arial, Helvetica, sans-serif'>$regest[conest]</font></td>";
      echo "<td><font size='1' face='Arial, Helvetica, sans-serif'>$regest[descripcion]</font></td>";
      echo "</tr>";

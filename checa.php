<?php
  ...
  if($_REQUEST[Credencial]<>''){				//Agrega producto por producto
     ...
     $Credencial = cZeros($_REQUEST[Credencial],5);
     
     $Mov     = array("entradai","entradaf","salidai","salidaf");

     $EmpA    = mysql_query("SELECT nombre,horario,id FROM emp WHERE credencial='$Credencial'");

     if(mysql_num_rows($EmpA) <> 0 ){
     		...
     		$DiaSem  = date("w",$FechaIni);
	  		...
	  		$HorasA  = mysql_query("SELECT * FROM hrd WHERE id='$Emp[horario]' AND dia='$DiaSem'");
		...
		//       if($Hora >= $cIni and $Hora <= $cLimite ){
            ...
            $Msj = "Tarjeta no existente/$Hora ";
     
     }		
  }else{
            $Msj = "La tarjeta No registrada/$Hora";
  }

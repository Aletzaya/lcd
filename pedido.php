<?php
  ...
  $op      = $_REQUEST[op];
  $Id      = 41; 										                 //Numero de query dentro de la base de datos
  ...
  if($busca >= 'a'){
     ...
     $cSql = "SELECT $Qry[campos] FROM el 
              WHERE $BusInt $Qry[filtro]";

  }else{
   
     $cSql = "SELECT $Qry[campos] FROM pedido WHERE pedido.id >= '$busca' $BusInt  $Qry[filtro]";
  ...
  if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           if($rg[status]=='CERRADA'){
              ...
              echo "<td align='center'><a href=javascript:wingral('imppedidopdf.php?busca=$rg[id]')><img src='lib/print.png' alt='Modifica rg' border='0'></td>";
              
           }else{
              echo "<td>&nbsp;</td>";
           }           
           echo "<td align='center'><a href='pedidod.php?busca=$rg[id]'><img src='lib/browse.png' alt='Modifica Registro' border='0'></td>";

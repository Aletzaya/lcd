 ...
 // $Medico=$_REQUEST[Medico];
  ...
  $NomI    = mysql_fetch_array($InstA);
	...
	if ($cero<>'No'){
		...
		$cSql="select orden,cliente,fecha,institucion,suc,pagada as ingreso,ot.status from ot
		where
...
<?php
  ...
  if(!$res=mysql_query($cSql,$link)){
        ...
        if($cero=='Todo'){
			     echo "&nbsp; &nbsp;<a href='comisionesinst.php?FecI=$FecI&FecF=$FecF&Institucion=$Institucion&sucursal=$_REQUEST[sucursal]&cero=No'><font size='2' face='Arial, Helvetica, sans-serif'><b>Todas las Ot's</b></a></p>";
        }else{

<?php
  ...
  $Hora      = date("h:i:s");
  $Titulo    = "Observaciones de envio [$Estudio]";
  ...
  if($_REQUEST[Boton] == Aceptar){        //Para agregar uno nuevo
        $lUp2   = mysql_query("UPDATE maqdet set obsenv='$_REQUEST[Observaciones]' WHERE orden='$orden' and estudio='$Estudio'");
...
echo "<tr><td align='center'><form name='form1' method='post' action='obsmqlenv.php?orden=$orden&Estudio=$Estudio'>";
echo "$Gfont Observaciones:&nbsp;";
//echo "$Gfont <strong>Observaciones:&nbsp;</strong>";
echo "<TEXTAREA NAME='Observaciones' cols='70' rows='3'>$registro[obsenv]</TEXTAREA>";
echo Botones2();
echo "</td></tr>"; 
echo "</table> <br>";
echo "</form>";

<?php
  ...
  if($_REQUEST[Boton] == Aceptar or $_REQUEST[Boton] == Aplicar ){        //Para agregar uno nuevo
	  ...
	  if($busca=="NUEVO"){
         ...
         $Fecha = date("Y-m-d");
         $Hora  = date("H:i");        
         $lUp   = mysql_query("INSERT INTO sl (fecha,hora,concepto,recibio,status,depto,unidad,almacen) VALUES ('$Fecha','$Hora','$_REQUEST[Concepto]','$_REQUEST[Recibio]','ABIERTA','$_REQUEST[Depto]','$_REQUEST[Unidad]','$_REQUEST[Almacen]')");
         ...
         $Fecha = date("Y-m-d");
         $Hora  = date("H:i"); 
         $lUp = mysql_query("UPDATE sl SET concepto='$_REQUEST[Concepto]',recibio='$_REQUEST[Recibio]',status='$_REQUEST[Status]',fecha='$Fecha',hora='$Hora',depto='$_REQUEST[Depto]',unidad='$_REQUEST[Unidad]',almacen='$_REQUEST[Almacen]' WHERE id='$busca' limit 1");
                      
        $lBd   = true;

        if($Cpo[status]<>'CERRADA' and $_REQUEST[Status]=='CERRADA'){

              $ProdB  = mysql_query("SELECT * FROM sl WHERE id='$busca'");
              $Prodb=mysql_fetch_array($ProdB);

              $ProdA  = mysql_query("SELECT * FROM sld WHERE id='$busca'");
              ...
              while($Prod=mysql_fetch_array($ProdA)){
                 ...
                 $Up = mysql_query("UPDATE invl SET existencia = existencia - $Prod[cantidad], $Prodb[almacen] = $Prodb[almacen] - $Prod[cantidad],costo = '$Inv[costo]' WHERE clave='$Prod[clave]' LIMIT 1");

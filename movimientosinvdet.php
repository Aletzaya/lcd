<?php
  ...
  $NomA=mysql_query("select * from invldet where invldet.producto='$clave' $sucinvt and date_format(invldet.fecha,'%Y-%m-%d')>='$FechaI' and date_format(invldet.fecha,'%Y-%m-%d')<='$FechaF' ORDER BY invldet.id DESC ",$link);
...
echo "<form name='form' method='post' action='movimientosinvdet.php?clave=$clave&suc=$suc&FechaI=$FechaI&FechaF=$FechaF'>";

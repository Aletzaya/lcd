<?php
  ...
  if(isset($_REQUEST[filtro]))  { $_SESSION['filtro'] = $_REQUEST[filtro];}     #Campo por el cual se ordena 
  if(isset($_REQUEST[filtro3]))  { $_SESSION['filtro3'] = $_REQUEST[filtro3];}     #Campo por el cual se ordena 
  if(isset($_REQUEST[filtro5]))  { $_SESSION['filtro5'] = $_REQUEST[filtro5];}     #Campo por el cual se ordena 
  if(isset($_REQUEST[filtro7]))  { $_SESSION['filtro7'] = $_REQUEST[filtro7];}     #Campo por el cual se ordena 
  if(isset($_REQUEST[filtro9]))  { $_SESSION['filtro9'] = $_REQUEST[filtro9];}     #Campo por el cual se ordena 
  ...
  $filtro7    = $_SESSION[filtro7];       
  $filtro9    = $_SESSION[filtro9];       
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a class='seleccionar' href='$cLink?busca=$rg[estudio]&pagina=$pagina'>editar</a></td>";
...
///</form>
      ...
      echo "<td>$Gfont <form name='form11' method='post' action='$_SERVER[PHP_SELF]?filtro=*&filtro3=*&filtro5=*&filtro7=*&filtro9=*'>";

      echo "<b>Contenido:</b></label>&nbsp;<input type='text' name='contenido' size='30' maxlength='50' placeholder='Estudio que contenga'> &nbsp; &nbsp; &nbsp; ";

      echo "<input type='hidden' name='pagina' value='1'>";   //Para saber cuando hay una busqueda nueva y pagina la ponga en 1;

      echo "<a  class='pg'  href='$_SERVER[PHP_SELF]?pagina=0&Sort=Asc&busca=&filtro=*&filtro3=*&filtro5=*&filtro7=*&filtro9=*&orden=est.estudio'>Limpia pantalla</a>";

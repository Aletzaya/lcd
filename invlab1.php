<?php
  ...
  if(substr($busca,0,1)=='-'){
     ...
     $cSql  = "SELECT $Qry[campos],invl.id FROM $Qry[froms] WHERE invl.clave='$busca' and invl.status='Activo'";
  }else{
  ...
  $aIzq   = array("Selecciona","-","-");	  //Arreglo donde se meten los encabezados; Izquierdos
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='left'><a class='ord' href='entlabd.php?Clave=$rg[clave]&Idproducto=$rg[id]'>$Gfont $rg[clave]</td>";

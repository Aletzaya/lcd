<?php
  ...
  $Titulo='Reporte General por Analito';
	...
	<table width="100%" border="0">
	  <tr>
    	<td><div align='center'>
    	    ...
    	    <font size="2"><?php echo "$Titulo"; ?><br><hr>
        	</div>
	    </td>
	  </tr>
	</table>
	...
	while($Est=mysql_fetch_array($cSql)) {
		...
		$contarest='';
			...
			if($contarest<>''){
					...
					$cSqlF=mysql_query("SELECT *
					FROM conest
					WHERE conest.conest = '$Est[estudio]' order by estudio");

					while($EstB=mysql_fetch_array($cSqlF)) {
/*
						$cSqlC   = mysql_query("SELECT otd.estudio,count(otd.estudio) as cnt
								   FROM otd,ot
								   WHERE ot.orden=otd.orden AND otd.estudio='$EstB[estudio]' and ot.fecha='$Fechai' and ($Sucursal)
								   ORDER BY otd.estudio");
*/

						$cSqlC=mysql_query("SELECT otd.estudio, count(otd.estudio) as cnt
						FROM otd, ot
						WHERE ot.fecha='$Fechai' and ot.orden=otd.orden and otd.estudio = '$EstB[estudio]' and ($Sucursal)
					...
					$contarCTT=$contarCTT+$contarCT;
			    	$contarCT='';
		
			}
	}

	echo "<tr><th align='CENTER' bgcolor='#a2b2de' colspan='2'><font size='2' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>Total de Analitos: $nRng</font></th>";
	echo "<th align='CENTER' bgcolor='#a2b2de'><font size='2' face='Verdana, Arial, Helvetica, sans-serif' color='#000000'>$contarCTT</font></th>";
	echo "</table>";

	echo "<div align='center'>";
	echo "<p align='center'><font face='verdana' size='-2'><a href='pidedatos.php?cRep=40'>";

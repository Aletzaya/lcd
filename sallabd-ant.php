<?php
  ...
  if($op=='ag' AND $Clave <> '' AND $_REQUEST[Cantidad]>0 ){
     ...
     $Up       = mysql_query("UPDATE invl SET existencia = existencia - $_REQUEST[Cantidad],
	 			$_REQUEST[Almacen] = $_REQUEST[Almacen] - $_REQUEST[Cantidad],costo = '$Inv[costo]'
     ...
     $Up     = mysql_query("UPDATE invl SET existencia=existencia + $Cnt[cantidad],
		 	 $Almacen = $Almacen + $Cnt[cantidad] WHERE clave='$Cnt[clave]' LIMIT 1");

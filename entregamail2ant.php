<?php
 ...
 // $ImagenesA  = mysql_query("SELECT * FROM envimglog WHERE envimglog.orden='$Orden'");
...
while ($row = mysql_fetch_array($ImgA)) {   
    ...
    echo "<td align='center' onMouseOver=this.style.backgroundColor='$Fdo';this.style.cursor='hand'
        onMouseOut=this.style.backgroundColor='#FFFFFF';><a class='pg' href='entregamail2.php?Orden=$Orden&Reg=$R&Archivo=$row[archivo]'><IMG SRC='estudios/$foto' border='0' width='70'>$Seleccionado<br>$Gfont $R - $row[archivo] <font></td>"; 

      if($R == 5 or $R==10 or $R==15 or $R==20 or $R==25 or $R==30 or $R==35 or $R==40 or $R==45 or $R==50 or $R==55 or $R==60 or $R==65 or $R==70 or $R==75  or $R==80 or $R==85 or $R==90 or $R==95 or $R==100){
        echo "</tr>";
        echo "<tr>";
      }
...
echo "<a class='pg' href='enviocorreoimg.php?Orden=$Orden&Correo=$rgp[mail]&Correom=$rgm[mail]&Correoi=$rgi[mail]&entemailpac=$He[entemailpac]&entemailmed=$He[entemailmed]&entemailinst=$He[entemailinst]'><font size='2'><b>Envio de Imagenes<b></font></a>";

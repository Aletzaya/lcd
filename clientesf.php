<?php
  ...
  #Variables comunes;
  $Titulo = "Clientes de facturacion";
  $op     = $_REQUEST[op];
  $Msj    = $_REQUEST[Msj];
  $Id     = 37; 
  ...
  $aIzq   = array("P/facturar","-","-"," ","-","-");	  //Arreglo donde se meten los encabezados; Izquierdos
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a class='Seleccionar' href='facturasd.php?busca=$rg[id]&Cliente=$rg[id]'>seleccionar</a></td>";
           ...
           echo "<td><a class='pg' href=javascript:winuni('cambiaclif.php?busca=$rg[id]')>".ucwords(strtolower($rg[nombre]))."</a></td>";
           echo "<td>$Gfont $rg[rfc]</font></td>";
           echo "<td>$Gfont ".ucwords(strtolower($rg[colonia]))."</font></td>";
           echo "<td>$Gfont ".ucwords(strtolower($rg[municipio]))."</font></td>";

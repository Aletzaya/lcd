...
echo "<table align='center' width='100%' border='0'>";
	...
	echo "<form name='form' method='post' action='repdemandainst4.php?pagina=$pagina&Sort=Asc&busca=$busca&filtro=$filtro&filtro3=$filtro3&filtro5=$filtro5&filtro7=$filtro7&filtro9=$filtro9&FechaI=$FechaI&FechaF=$FechaF'>";
	
	echo "<td>&nbsp; $Gfont <font size='1'><b> DE: $FechaI </b></font><input type='hidden' readonly='readonly' name='FechaI' size='10' value ='$FechaI'> &nbsp; <img src='lib/calendar.png' border='0' onclick=displayCalendar(document.forms[0].FechaI,'yyyy-mm-dd',this)></td>";
		
	echo "<td>&nbsp; $Gfont <font size='1'><b> A: $FechaF </b></font><input type='hidden' readonly='readonly' name='FechaF' size='10' value ='$FechaF'> &nbsp; <img src='lib/calendar.png' border='0' onclick=displayCalendar(document.forms[0].FechaF,'yyyy-mm-dd',this)></td>";
    ...
    $Tit = "<tr bgcolor='#a2b2de'><td align='center'>$Gfon Institucion </td><td align='center' colspan='2'>$Gfon Nombre</td>
...
//                    $cTit = $cTit . "<td align='center'>$Gfon ".number_format($aCnt[$x],'0')."</font></td>";
				...
				<td align='center'><a href=javascript:winuni('comisionesmed.php?FecI=$FechaI&FecF=$FechaF&Institucion=*&$Sucursal=1&Promotor=$filtro5&Medico=$Med')>$Gfont $Inst</a>
				</td><td width='22%'>$Gfont $Nombre </font></td><td align='rigth' width='5%'>$Gfont <font size='1'>$Status</font></td><td align='center' width='15%'>$Gfont $Esp</font></td><td>$Gfont $Promotor</font></td>";
                echo $cTit . "<td align='right'>$Gfont ".number_format($SubT,'0')."</td><td align='right'>$Gfont ".number_format($Promedio,'2')." </td>
...
//                    $cTit = $cTit . "<td align='center'>$Gfon ".number_format($aCnt[$x],'0')."</font></td>";
	...
	$OtB  = mysql_fetch_array(mysql_query($cOtB,$link));
	echo "<tr bgcolor='$Fdo' onMouseOver=this.style.backgroundColor='$Gbarra';this.style.cursor='hand' onMouseOut=this.style.backgroundColor='$Fdo';>
	<td align='center'><a href=javascript:winuni('comisionesmed.php?FecI=$FechaI&FecF=$FechaF&Institucion=*&$Sucursal=1&Promotor=$filtro5&Medico=$Med')>$Gfont $Inst</a></td>
	<td>$Gfont $Nombre</font></td><td aling='rigth'>$Gfont <font size='1'>$Status</font></td><td align='center'>$Gfont $Esp</font></td><td>$Gfont $Promotor</font></td>";
	echo $cTit . "<td align='right'>$Gfont ".number_format($SubT,'0')."</td><td align='right'>$Gfont ".number_format($Promedio,'2')." </td>
    ...
    for ($i = $Ini; $i <= $Fin; $i = $i + 1) {
        ...
        $cTit = $cTit . "<td align='right'>$Gfon2 <b>".number_format($tCnt[$x],'0')."</b></font></td>";
    }
	$PromedioG= $GraT/$Cmes;
	$VentaT+=$Venta;
    ...
    echo $cTit. "<td align='right'>$Gfon2 <b>".number_format($GraT,'0')."</b></td>
	<td align='right'>$Gfon2 <b>".number_format($PromedioG/$GraT,'2')."</b>

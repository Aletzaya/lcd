<?php
  ...
  if( $busca < 'a'){
     $cSql="SELECT $Qry[campos],ot.ubicacion,ot.encaja,ot.status,ot.pagada FROM cli,$Qry[froms] WHERE ot.cliente=cli.cliente and ot.fecha>='$Fech2' and ot.fecha<='$Fech3' AND ot.orden >= '$busca' $Qry[filtro]";
  }else{
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      $res  = mysql_query($sql);
		...
		echo "<form name='form' method='post' action='ordenescon3.php?Fech2=$Fech2&Fech3=$Fech3'>";

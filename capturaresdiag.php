...
//             if(substr($Otd[tres],0,4)=='0000'){ //Actualizo la fecha y hora del paso 2, que es imprisiion de et.;
                ...
                if($Otd[lugar] <= '4' OR $Otd[lugar] == '' ){
                   ...
                   $Up = mysql_query("UPDATE otd set tres = '$Fecha $Hora', lugar='4', status='TERMINADA',capturo='$Usr',alterno='$alterno',fr='$fr2' 
...
function elegir() {
 ...
 alert('no se borra nada');
    ...
    $OtdA   = mysql_query("SELECT status,capturo,tres,usrvalida,fechavalida,impest,impres FROM otd WHERE estudio='$estudio' AND orden='$busca'");
    $Otd    = mysql_fetch_array($OtdA);

<?php
  ...
  if($_REQUEST[Boton] == Agregar){
	  ...
	  $lUp   = mysql_query("INSERT INTO elealt (estudio,descripcion,tipo,longitud,decimales,id,min,max) 
	   		  VALUES 
  	  ...
  	  $lUp   = mysql_query("UPDATE elealt SET descripcion='$_REQUEST[Descripcion]',tipo='$_REQUEST[Tipo]',
     ...
     $lUp   = mysql_query("DELETE FROM elealt WHERE estudio='$busca' AND id='$_REQUEST[cId]' LIMIT 1");
     $Msj   = "El registro: $_REQUEST[cId] a sido dado de baja";

  }elseif($op=="rp"){			//Genera el archivo para imprimir los resultados

     $Reporte = "informesalt/".$busca.".php";
     ...
     if (file_exists($Reporte)) {
      ...
      fwrite($fp,"^EleA=mysql_query(\"select * from elealt where estudio='^Estudio' order by id\",^link);\n");
      ...
      $EleA=mysql_query("select * from elealt where estudio='$Estudio' order by id",$link);
	  ...
	  while ($Ele=mysql_fetch_array($EleA)){
   	...
   	fwrite($fp, "?>\n");
    	...
    	header("Location: estudioselealt.php?Msj=El estudio $busca ha sido creado con exito!!!");
  ...
  $cSql     = "SELECT * FROM elealt WHERE estudio = '$busca'";
  ...
  if(!$res=mysql_query($cSql.$cWhe,$link)){
      ...
      cMensaje("No se encontraron resultados ò hay un error en el filtro");    #Manda mensaje de datos no existentes
		...
		while($registro=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a href=elementosalt.php?cEd=$registro[idnvo]&pagina=$pagina><img src='lib/edit.png' alt='Edita reg' border='0'></td>";
           ...
           echo "<td align='center'><a href=elementosalt.php?cId=$registro[idnvo]&op=El&pagina=$pagina><img src='lib/deleon.png' alt='Borra reg' border='0'></td>";
           echo "</tr>";
           $nRng++;
		}//fin while
   ...
   echo "<form name='form1' method='get' action=".$_SERVER['PHP_SELF']." onSubmit='return ValCampos();'>";
      ...
      if($_REQUEST[cEd]<>''){
   		$CpoA  = mysql_query("SELECT * FROM elealt WHERE idnvo='$_REQUEST[cEd]'");
...
echo " &nbsp;  &nbsp; <a class='pg' href=estudioselealt.php><img src='lib/regresa.jpg' border=0></a> &nbsp; ";
echo "<a class='pg' href='elementosalt.php?op=rp'>$Gfont Genera reporte $Gfon </a>";

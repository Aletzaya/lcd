...
class auth{
	...
	var $USERNAME = "Admon";	// Change this to the proper DB USERNAME
   ...
   $Fecha=date("Y-m-d ");
      ...
      $UpdateRecords = "UPDATE authuser SET lastlogin = NOW(), logincount = logincount + 1 WHERE uname='$username'";
		...
		$access1     = mysql_query("SELECT * FROM authuser WHERE uname='$username'");

		$accesso = mysql_fetch_array($access1);

		$Upinto = mysql_query("INSERT INTO logacceso (id,uname,team,level,fechacc,ipacc) VALUES ('$accesso[id]','$accesso[uname]','$accesso[team]','$accesso[level]',NOW(),'$ipaddress')");

<?php
  ...
  if(substr($busca,0,1)=='-'){
     ...
     $cSql  = "SELECT $Qry[campos],invl.id FROM $Qry[froms] WHERE invl.clave='$busca'";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos],invl.id FROM $Qry[froms] WHERE $BusInt $Qry[filtro]";
  }   
  //echo $cSql;
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
    ...
    echo "<a href='estudiose2.php?busca=$estudio&opm=equipos&suc=$suc'><img src='lib/regresa.jpg' border='0' height='30'></a>";
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a class='seleccionar' href='estudiose2.php?Producto=$rg[clave]&busca=$estudio&opm=equipos&suc=$suc&idproducto=$rg[id]'>$Gfont seleccionar</td>";
    ...
    CuadroInferiorinv6($busca);      #-------------------Siempre debe de estar por que cierra la tabla principal .

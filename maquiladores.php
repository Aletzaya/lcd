<?php
  ...
  #Variables comunes;
  $Msj     = $_REQUEST[Msj];
  $Titulo = "Catalogo de maquiladores";
  $op     = $_REQUEST[op];
  $Id     = 47; 										                 //Numero de query dentro de la base de datos
  
  #Intruccion a realizar si es que mandan algun proceso
  if($op=='Si'){                    //Elimina Registro
      $lUp=mysql_query("DELETE FROM mql WHERE id='$_REQUEST[cId]' limit 1",$link);
  ...
  if($busca >= 'a'){
     ...
     if($Palabras > 1){
        ...
        for ($i = 0; $i < $Palabras; $i++) {
            if(!isset($BusInt)){$BusInt=" mql.nombre like '%$P[$i]%' ";}else{$BusInt=$BusInt." and mql.nombre like '%$P[$i]%' ";}
        }
     }else{
        $BusInt=" mql.nombre like '%$busca%' ";  
     }
     ...
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE mql.id >='$busca' $BusInt  $Qry[filtro]";

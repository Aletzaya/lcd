...
if (isset($_REQUEST['submitted']) AND $lBd ) {
  ...
  if($op=='rs'){  //Registra resultados
      ...
      $Msj = "";
		$OtdA   = mysql_query("SELECT status,capturo,tres,usrvalida,fechavalida,impest,impres,fr FROM otd WHERE estudio='$estudio' AND orden='$busca'");
	...
	if($Otd[capturo]<>''){
       ...
       if($_REQUEST[Confirmacion]<>''){
		 ...
		 if($Acc[uname] == $Usr){     
				...
				$Up = mysql_query("UPDATE otd SET fechavalida = '$Fecha $Hora', usrvalida='$Usr',alterno='$alterno',fr='$fr2',creapdf='html'
			...
			$Msj = $Msj . "<br>Lo siento! tu confirmacion no es correcta</br>";			    	
                ...
                $Up = mysql_query("UPDATE otd SET tres = '$Fecha $Hora', status='TERMINADA', capturo='$Usr',alterno='$alterno',fr='$fr2',creapdf='html'
	                   WHERE orden='$busca' and estudio='$estudio'");   
	}
...
function elegir() {
 ...
 alert('no se borra nada');
	...
	if($alterno==0){				
    	...
    	echo "<div align='center'>$Gfont <font color='#330099'><b> Captura Standar: <a class='pg' href='capturaresdiagvalida.php?busca=$busca&estudio=$estudio&alterno=0'><img src='lib/iconfalse.gif'></b></a>";
    }   

	if($alterno==1){				
       	echo "$Gfont <font color='#330099'><b> Captura Alternativa: <img src='lib/slc.png'>";
    }else{
    	echo "$Gfont <font color='#330099'><b> Captura Alternativa: <a class='pg' href='capturaresdiagvalida.php?busca=$busca&estudio=$estudio&alterno=1'><img src='lib/iconfalse.gif'></b></a>";
    }   

	if($alterno==2){				
       	echo "$Gfont <font color='#330099'><b> Captura Alternativa (2): <img src='lib/slc.png'></div><br>";
    }else{
    	echo "$Gfont <font color='#330099'><b> Captura Alternativa (2): <a class='pg' href='capturaresdiagvalida.php?busca=$busca&estudio=$estudio&alterno=2'><img src='lib/iconfalse.gif'></b></a></div><br>";
    }   

    echo "<form name='form1' method='post' action='capturaresdiagvalida.php?alterno=$alterno'>";
    ...
    $OtdA = mysql_query("SELECT ot.orden, ot.fecha,otd.estudio,est.descripcion,est.depto,otd.alterno,otd.capturo,otd.usrvalida,otd.fr

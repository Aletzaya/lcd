<?php
  ...
  #Variables comunes;
  $Msj     = $_REQUEST[Msj];
  $Titulo = "Catalogo de tecnicas";
  $op     = $_REQUEST[op];
  $Id     = 52; 										                 //Numero de query dentro de la base de datos
  
  #Intruccion a realizar si es que mandan algun proceso
  if($op=='Si'){                    //Elimina Registro
      $lUp=mysql_query("DELETE FROM tec WHERE id='$_REQUEST[cId]' limit 1",$link);
  ...
  if($busca >= 'a'){
     ...
     if($Palabras > 1){
        ...
        for ($i = 0; $i < $Palabras; $i++) {
            if(!isset($BusInt)){$BusInt=" tec.tecnica like '%$P[$i]%' ";}else{$BusInt=$BusInt." and tec.tecnica like '%$P[$i]%' ";}
        }
     }else{
        $BusInt=" eqp.tecnica like '%$busca%' ";  
     }
     ...
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE tec.id >='$busca' $BusInt  $Qry[filtro]";
...
<?php
 ...
 headymenu($Titulo,1);
  ...
  if(!$res=mysql_query($cSql,$link)){
    ...
    CuadroInferior5($busca);      #-------------------Siempre debe de estar por que cierra la tabla principal .

...
*/
 if($filtro3=='*'){
  ...
  $Titulo = "Reporte de Productividad del $FechaI al $FechaF";
    	...
    	$cOtA="select otd.usrest as usuario,date_format(ot.fecha,'%Y-%m') as fecha,count(*),ot.orden,ot.suc,ot.institucion,otd.estudio,est.depto,sum(otd.precio) as precios,sum(otd.precio*(otd.descuento/100)) as descuentos
    	from otd,ot,est
...
<body bgcolor="#FFFFFF">
  ...
  echo "<form name='form' method='post' action='repoperativo3.php?pagina=$pagina&Sort=Asc&busca=$busca&filtro=$filtro&filtro3=$filtro3&filtro5=$filtro5&filtro7=$filtro7&filtro9=$filtro9&FechaI=$FechaI&FechaF=$FechaF'>";
...
//                    $cTit = $cTit . "<td align='center'>$Gfon ".number_format($aCnt[$x],'0')."</font></td>";
          ...
          onMouseOut=this.style.backgroundColor='$vist';>$Gfont ".number_format($aCntot[$x],'2')."</font></td>";
              ...
              if($capt==''){
				...
				<td align='center'><a href=javascript:winuni('repoperativod3.php?FecI=$FechaI&FecF=$FechaF&Institucion=$filtro5&Suc=$filtro3&capt=$capt&depto=$filtro7&etapa=$filtro9')>$Gfont <b>$capt</b></a>
				</td></td>";

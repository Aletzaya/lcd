...
$cSql="SELECT maqdet.orden,maqdet.estudio,maqdet.mint,maqdet.fenv,maqdet.henv,maqdet.obsenv,ot.orden as ord,ot.suc,ot.institucion,ot.cliente,cli.cliente,cli.nombrec,cli.fechan,cli.sexo,est.estudio as estud,est.descripcion,maqdet.usrrecint
FROM maqdet, ot, cli, est
WHERE maqdet.orden=ot.orden and maqdet.estudio = est.estudio AND ot.cliente = cli.cliente and maqdet.fenv>='$FecI' and maqdet.fenv <='$FecF' and maqdet.mint<>ot.suc
...
while($rg=mysql_fetch_array($UpA)) {
  ...
  echo "<td align='center'><font size='1' face='Arial, Helvetica, sans-serif'>$rg[usrrecint]</font></td>";
	echo "</tr>";
	$Estudios++;
}

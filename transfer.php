<?php
  ...
  $op      = $_REQUEST[op];
  $Id      = 40; 										                 //Numero de query dentro de la base de datos
  ...
  if($busca >= 'a'){
     ...
     if($Palabras > 1){
        ...
        for ($i = 0; $i < $Palabras; $i++) {
            if(!isset($BusInt)){$BusInt=" trans.mov like '%$P[$i]%' ";}else{$BusInt=$BusInt." AND trans.mov like '%$P[$i]%' ";}
        }
     }else{
        $BusInt=" trans.mov like '%$busca%' ";  
     }

     #Armo el query segun los campos tomados de qrys;
     $cSql = "SELECT $Qry[campos] FROM trans 
              WHERE $BusInt $Qry[filtro]";

  }else{
   
     $cSql = "SELECT $Qry[campos] FROM trans WHERE trans.id >= '$busca' $BusInt  $Qry[filtro]";
  ...
  if(!$res=mysql_query($cSql)){
      ...
      while($rg=mysql_fetch_array($res)){
           ...
           if($rg[status]=='CERRADA'){
              //echo "<td align='center'><a href='impsallab.php?busca=$rg[id]'><img src='lib/print.png' alt='Imprime' border='0'></td>";
              echo "<td align='center'><a href=javascript:wingral('imptransferpdf.php?busca=$rg[id]')><img src='lib/print.png' alt='Modifica rg' border='0'></td>";
              
           }else{
              echo "<td>&nbsp;</td>";
           }           
           echo "<td align='center'><a href='transferd.php?busca=$rg[id]'><img src='lib/browse.png' alt='Modifica Registro' border='0'></td>";

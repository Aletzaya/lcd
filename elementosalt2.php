<?php
  ...
  if($_REQUEST[Boton] == Agregar){
	  ...
	  $lUp   = mysql_query("INSERT INTO elealt2 (estudio,descripcion,tipo,longitud,decimales,id,min,max) 
	   		  VALUES 
	   		  ('$busca','$_REQUEST[Descripcion]','$_REQUEST[Tipo]','$_REQUEST[Longitud]','$_REQUEST[Decimales]',
	   		  '$_REQUEST[Id]','$_REQUEST[Min]','$_REQUEST[Max]')");	  	 
  	 
  }elseif($_REQUEST[Boton] == Actualizar){
  	
  	  $lUp   = mysql_query("UPDATE elealt2 SET descripcion='$_REQUEST[Descripcion]',tipo='$_REQUEST[Tipo]',
  	           longitud='$_REQUEST[Longitud]',decimales='$_REQUEST[Decimales]',id='$_REQUEST[Id]',
  	           min='$_REQUEST[Min]',max='$_REQUEST[Max]'
  	  		     WHERE idnvo='$_REQUEST[Up]'");
  	
  }elseif($op=='Si'){                    //Elimina Registro

     $lUp   = mysql_query("DELETE FROM elealt2 WHERE estudio='$busca' AND id='$_REQUEST[cId]' LIMIT 1");
     $Msj   = "El registro: $_REQUEST[cId] a sido dado de baja";

  }elseif($op=="rp"){			//Genera el archivo para imprimir los resultados

     $Reporte = "informesalt2/".$busca.".php";
     ...
     if (file_exists($Reporte)) {
      ...
      fwrite($fp,"^EleA=mysql_query(\"select * from elealt2 where estudio='^Estudio' order by id\",^link);\n");
      ...
      $EleA=mysql_query("select * from elealt2 where estudio='$Estudio' order by id",$link);
	  ...
	  while ($Ele=mysql_fetch_array($EleA)){
   	...
   	fwrite($fp, "?>\n");
    	...
    	header("Location: estudioselealt2.php?Msj=El estudio $busca ha sido creado con exito!!!");
  ...
  $cSql     = "SELECT * FROM elealt2 WHERE estudio = '$busca'";

  $Tabla    = "elealt2";

  $Titulo   = "Elementos del estudio Alterno 2: [$busca]";
  ...
  if(!$res=mysql_query($cSql.$cWhe,$link)){
      ...
      cMensaje("No se encontraron resultados ò hay un error en el filtro");    #Manda mensaje de datos no existentes
		...
		while($registro=mysql_fetch_array($res)){
           ...
           echo "<td align='center'><a href=elementosalt2.php?cEd=$registro[idnvo]&pagina=$pagina><img src='lib/edit.png' alt='Edita reg' border='0'></td>";
           ...
           echo "<td align='center'><a href=elementosalt2.php?cId=$registro[idnvo]&op=El&pagina=$pagina><img src='lib/deleon.png' alt='Borra reg' border='0'></td>";
           echo "</tr>";
           $nRng++;
		}//fin while
   ...
   echo "<form name='form1' method='get' action=".$_SERVER['PHP_SELF']." onSubmit='return ValCampos();'>";
      ...
      if($_REQUEST[cEd]<>''){
   		$CpoA  = mysql_query("SELECT * FROM elealt2 WHERE idnvo='$_REQUEST[cEd]'");
      ...
      echo "<td>$Gfont ";	    		 
          ...
          echo "<input type='TEXT' name='Max' value='$Cpo[max]' size='3' maxlength='9')> &nbsp; ";
			 ...
			 /*
          //echo "Descripcion: ";
          //echo "<input type='TEXT' name='Descripcion' value='$Cpo[descripcion]' size='20' maxlength='40')> &nbsp; ";

          echo "Produce: ";
          echo "<select name='Produce'>";
          echo "<option value='Kilos'>Kilos</option>";
          echo "<option value='Metros'>Metros</option>";
          echo "<option selected value='$Cpo[produce]'>$Cpo[produce]</option>";             
          echo "</select> &nbsp; ";           

          echo "Prod/dia: ";
          echo "<input type='TEXT' name='Produccion' value='$Cpo[produccion]' size='5' maxlength='7')> &nbsp; ";

          echo "Hrs/trabajo: ";
          echo "<input type='TEXT' name='Horas' value='$Cpo[horas]' size='3' maxlength='5')>";
          */			 
...
echo " &nbsp;  &nbsp; <a class='pg' href=estudioselealt2.php><img src='lib/regresa.jpg' border=0></a> &nbsp; ";
echo "<a class='pg' href='elementosalt2.php?op=rp'>$Gfont Genera reporte $Gfon </a>";

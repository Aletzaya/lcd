<?php
  ...
  if($op=='Si'){                    //Elimina Registro
  	  ...
  	  $lUp = mysql_query("DELETE FROM cmc WHERE id='$_REQUEST[cId] LIMIT 1'");
  ...
  if($Palabras > 1){
     ...
     for ($i = 0; $i < $Palabras; $i++) {
            if(!isset($BusInt)){$BusInt=" cmc.medico like '%$P[$i]%' ";}else{$BusInt=$BusInt." AND cmc.medico like '%$P[$i]%' ";}
     }
  }else{
     $BusInt=" cmc.medico like '%$busca%' ";  
  }
  
  
  if(substr($busca,0,1)=='-'){
     $busca=substr($busca,1);
     $cSql="SELECT $Qry[campos] FROM $Qry[froms] WHERE cmc.medico='$busca'";
  }else{
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      $sql  = $cSql.$cWhe." ORDER BY ".$orden.",cmc.mes $Sort LIMIT ".$limitInf.",".$tamPag;
      //echo $sql;
      $res  = mysql_query($sql);

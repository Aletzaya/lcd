<?php
  ...
  if($Palabras > 1){
     ...
     for ($i = 0; $i < $Palabras; $i++) {
            if(!isset($BusInt)){$BusInt=" logacceso.uname like '%$P[$i]%' ";}else{$BusInt=$BusInt." and logacceso.uname like '%$P[$i]%' ";}
     }
  }else{
     $BusInt=" logacceso.uname like '%$busca%' ";  
  }
  
  if( $busca == ''){
     $cSql="SELECT $Qry[campos] 
	 FROM $Qry[froms] WHERE date_format(logacceso.fechacc,'%Y-%m-%d')>='$Fech2' and date_format(logacceso.fechacc,'%Y-%m-%d')<='$Fech3'";
  }elseif( $busca < 'a'){
     $cSql="SELECT $Qry[campos]
	 FROM $Qry[froms] WHERE logacceso.id >= '$busca' $Qry[filtro]";
  }else{
     #Armo el query segun los campos tomados de qrys;
     $cSql="SELECT $Qry[campos] 
	 FROM $Qry[froms] WHERE $BusInt $Qry[filtro]";
  }   
  //echo $cSql;
...
<?php
 ...
 if(!$res=mysql_query($cSql)){
      ...
      $res  = mysql_query($sql);
		...
		echo "<form name='form' method='post' action='logacceso.php?Fech2=$Fech2&Fech3=$Fech3'>";

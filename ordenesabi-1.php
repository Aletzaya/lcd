    ...
    if ($DesctoS == "S") {
        ...
        if ($Sucursal <> '*') {
            ...
            $cSql = "SELECT ot.orden, ot.fecha, cli.nombrec, cli.afiliacion, otd.estudio, est.descripcion, otd.precio, 
                     ...
                     ot.descuento as descto,est.estpropio,est.muestras,est.entord,ot.horae,est.clavealt
            ...
            $cSql   = "SELECT ot.orden, ot.fecha, cli.nombrec, cli.afiliacion, otd.estudio, est.descripcion, otd.precio, 
                       ...
                       est.entord,ot.horae,est.clavealt
                       FROM ot, cli, otd, est, med
                       WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio AND ot.fecha>='$Fechai' 
                       AND ot.fecha <='$Fechaf' AND ot.medico=med.medico AND ot.servicio='$Serv' AND ot.descuento <> ' '
                       order by ot.orden";
        }
        ...
        if ($Sucursal <> '*' ) {
            ...
            $cSql = "SELECT ot.orden, ot.fecha, cli.nombrec, cli.afiliacion, otd.estudio, est.descripcion, otd.precio, 
                      ...
                      FROM ot, cli, otd, est, med
                      WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio AND ot.fecha>='$Fechai' 
                      AND ot.fecha <='$Fechaf' AND ot.medico=med.medico AND ot.suc='$Sucursal' AND ot.servicio='$Serv'
                      order by ot.orden";
        } else {
            ...
            $cSql   = "SELECT ot.orden, ot.fecha, cli.nombrec, cli.afiliacion, otd.estudio, est.descripcion, otd.precio, 
                       ...
                       FROM ot, cli, otd, est, med
                       WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio AND ot.fecha>='$Fechai' and
                       ot.fecha <='$Fechaf' AND ot.medico=med.medico AND ot.servicio='$Serv'
                       order by ot.orden";
        }
    }
    ...
    if ($DesctoS == "S") {
        ...
        if ($Sucursal <> '*') {
            ...
            $cSql   = "SELECT ot.orden, ot.fecha, cli.nombrec, cli.afiliacion, otd.estudio, est.descripcion, otd.precio, 
                       ...
                       ot.horae,est.clavealt
                       FROM ot, cli, otd, est, med
                       WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio AND ot.fecha>='$Fechai' and
                       ot.fecha <='$Fechaf' AND ot.medico=med.medico AND ot.suc='$Sucursal' AND ot.descuento <> ' '
                       order by ot.orden";
        } else {
            ...
            $cSql   = "SELECT ot.orden, ot.fecha, cli.nombrec, cli.afiliacion, otd.estudio, est.descripcion, otd.precio, 
                       ...
                       FROM ot, cli, otd, est, med
                       WHERE ot.cliente = cli.cliente AND ot.orden = otd.orden AND otd.estudio = est.estudio AND ot.fecha>='$Fechai' and
                       ot.fecha <='$Fechaf' AND ot.medico=med.medico AND ot.descuento <> ' '
                       order by ot.orden";
        }
...
<?php
    ...
    while ($rg = mysql_fetch_array($UpA)) {
            ...
            echo "<tr>";
            echo "<th>";
            echo "<th align='right'><font size='1' face='Arial, Helvetica, sans-serif'>$rg[estudio]&nbsp; $rg[clavealt] &nbsp;</font></th>";

<?php
  ...
  $Titulo     = "Detalle de personal de laboratorio";

  if($_REQUEST[Boton] == Aceptar or $_REQUEST[Boton] == Aplicar ){        //Para agregar uno nuevo

      if($busca=='NUEVO'){

         $lUp = mysql_query("INSERT INTO perlab (nombre,profesion,cedula,status,alias)
                VALUES
                ('$_REQUEST[Nombre]','$_REQUEST[Profesion]','$_REQUEST[Cedula]','$_REQUEST[Status]','$_REQUEST[Alias]')");

        $busca = mysql_insert_id();

       }else{

         $lUp = mysql_query("UPDATE perlab SET nombre='$_REQUEST[Nombre]',profesion='$_REQUEST[Profesion]',
                cedula='$_REQUEST[Cedula]',status='$_REQUEST[Status]',alias='$_REQUEST[Alias]'
                WHERE id='$busca' limit 1");
       }


      if($_REQUEST[Boton] == Aceptar){
         header("Location: perlab.php");
      }
      ...
      $cSql="SELECT * FROM perlab WHERE id='$busca'";
...
echo '<table width="100%" border="0">';
    ...
    echo "<tr>";
      ...
      echo "<td  width='10%' valign='center'>";
         ...
         echo "<a href='perlab.php'><img src='lib/regresa.jpg' border='0'></a>";

      echo "</td>";

      echo "<td align='center'> $Gfont ";

      echo "<form name='form1' method='get' action='perlabe.php' onSubmit='return Completo();'>";
             ...
             echo "<tr><td align='right'>Status:</td><td>";

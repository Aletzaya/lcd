<?php
  ...
  if($_REQUEST[Boton] == Agregar){
    ...
    $lUp   = mysql_query("INSERT INTO elepdf (estudio,descripcion,tipo,unidad,longitud,decimales,id,min,max,nota,vlogico,vtexto,alineacion,celda1,celda2,celda3,celdas,valref,idvalor1,parentesis1,valor1,operador1,idvalor2,valor2,parentesis2,operador2,idvalor3,valor3,parentesis3,operador3,idvalor4,valor4,parentesis4,calculo,condiciona) 
      ...
      $lUp2   = mysql_query("UPDATE elepdf SET fecha='$Fecha', usr='$Usr' WHERE estudio='$_REQUEST[busca]'");

      $lUp3  = mysql_query("INSERT INTO logelepdf (fecha,usr,estudio,concepto) VALUES
    ('$Fecha','$Usr','$_REQUEST[busca]','Agrego elemento $_REQUEST[Id]')");

  }elseif($_REQUEST[Boton] == Actualizar){
    
      $lUp   = mysql_query("UPDATE elepdf SET descripcion='$_REQUEST[Descripcion]',tipo='$_REQUEST[Tipo]',unidad='$_REQUEST[Unidad]',
    ...
    ('$Fecha','$Usr','$_REQUEST[busca]','Actualizo elemento $_REQUEST[Id]')");
     ...
     $lUp   = mysql_query("DELETE FROM elepdf WHERE estudio='$busca' AND id='$_REQUEST[cId]' LIMIT 1");
    ...
    ('$Fecha','$Usr','$busca','Elimina elemento $_REQUEST[cId]')");
          ...
          $lUp   = mysql_query("UPDATE elepdf SET bloqueado='Si', fecha='$Fecha', usr='$Usr' WHERE estudio='$_REQUEST[busca]'");
    ...
    ('$Fecha','$Usr','$_REQUEST[busca]','Cierra Estudio')");
          ...
          $lUp   = mysql_query("UPDATE elepdf SET bloqueado='No', fecha='$Fecha', usr='$Usr' WHERE estudio='$_REQUEST[busca]'");
  ...
  $cSql     = "SELECT * FROM elepdf WHERE estudio = '$busca'";

  $Tabla    = "elepdf";

  $Titulo   = "Elementos del estudio Standard: [$busca]";
...
<?php headymenu($Titulo,1);
      ...
      cMensaje("No se encontraron resultados ò hay un error en el filtro");    #Manda mensaje de datos no existentes
       ...
       $bloqueado='No';
        ...
        if($registro[tipo]=="c"){$cTt="Caracter";
          ...
          echo "<td align='center'><a href=elementospdf.php?cEd=$registro[idnvo]&pagina=$pagina><img src='lib/edit.png' alt='Edita reg' border='0'></td>";            
           ...
           if($bloqueado=='No'){
              echo "<td align='center'><a href=elementospdf.php?cId=$registro[id]&op=Si&pagina=$pagina><img src='lib/deleon.png' alt='Borra reg' border='0'></td>";
            }else{
  ...
  echo " &nbsp;  &nbsp; <a class='pg' href=estudioselepdf.php><img src='lib/regresa.jpg' border=0></a> &nbsp; ";
  //echo "<a class='pg' href='elementos.php?op=rp'>$Gfont Genera reporte $Gfon </a>";

  echo " &nbsp;  &nbsp;  &nbsp;  &nbsp; <a href=javascript:wingral('resultapdf.php?clnk=$clnk&Estudio=$busca&alterno=0')><font size='1'><img src='pdfenv.png' alt='pdf' border='0'></font></a>  ";

    if($bloqueado=='No'){

        echo " &nbsp;  &nbsp;  &nbsp;  &nbsp; <a href=elementospdf.php?busca=$busca&op=cerrar&pagina=$pagina>Cerrar <img src='images/candadoa.png' alt='pdf' border='0'></a><font size='2'> Ultima Modificacion por: $usr - $fecha </font>";

    }else{

        echo " &nbsp;  &nbsp;  &nbsp;  &nbsp; <a href=elementospdf.php?busca=$busca&op=abrir&pagina=$pagina>Abrir <img src='images/candadoc.png' alt='pdf' border='0'></a><font size='2'> Ultima Modificacion por: $usr - $fecha </font>";
    }
    
     echo " &nbsp;  &nbsp;  &nbsp;  &nbsp; <a href=javascript:wingral('logelepdf.php?busca=$busca&alterno=0')><font size='1'> *** Modificaciones ***</a>  ";
   ...
   echo "<form name='form1' method='get' action=".$_SERVER['PHP_SELF']." onSubmit='return ValCampos();'>";
      ...
      if($_REQUEST[cEd]<>''){
         $CpoA  = mysql_query("SELECT * FROM elepdf WHERE idnvo='$_REQUEST[cEd]'");

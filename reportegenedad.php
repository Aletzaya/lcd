<?php
  ...
  if($Ins == "*" OR $Ins==''){
     ...
     $OtA = mysql_query("SELECT cli.fechan, cli.sexo, count(*) as cantidad
     FROM cli, ot WHERE ot.fecha BETWEEN '$FechaI' AND '$FechaF' AND ot.cliente=cli.cliente
     GROUP BY cli.fechan, cli.sexo");
          			    
     $Titulo = "Clientes por entidad edad / genero del $FechaI del $FechaI al $FechaF";
			    
  }else{

     $OtA = mysql_query("SELECT cli.fechan, cli.sexo,count(*) as cantidad
     FROM cli, ot WHERE ot.fecha BETWEEN '$FechaI' AND '$FechaF' AND ot.cliente=cli.cliente AND ot.institucion='$Ins'
     GROUP BY cli.fechan, cli.sexo");
   
     $cInsA = mysql_query("SELECT alias FROM inst WHERE institucion='$Ins'");
     $cIns  = mysql_fetch_array($cInsA);

     $Titulo = "Clientes por entidad edad / genero del $FechaI al $FechaF Institucion: $cIns[alias]";
...
<?php
  ...
  echo "<td>$Gfont Edad</td>";
  echo "<td>$Gfont Genero</td>";
  ...
  while($rg = mysql_fetch_array($OtA)){
      ...
      echo "<tr bgcolor='$Fdo' onMouseOver=this.style.backgroundColor='$Gbarra';this.style.cursor='hand' onMouseOut=this.style.backgroundColor='$Fdo';>";
		echo "<td>$Gfont $anos</td>";
		echo "<td>$Gfont $rg[sexo]</td>";
//		echo "<td>$Gfont $rg[locconsultorio]</td>";
  ...
  echo "<form name='form1' method='post' action='reportegenedad.php'>";

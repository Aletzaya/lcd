...
<?php
  ...
  $NomI    = mysql_fetch_array($InstA);
	...
	if ($Sucursal <> '*') {
		...
		$cSql="SELECT cja.id,cja.fecha as fechas,cja.orden,cja.hora,cja.importe,cja.tpago,ot.importe,ot.status,cja.usuario,ot.institucion,ot.fecha,ot.hora as horaorden,ot.suc
				   FROM cja,ot
				   WHERE cja.fecha>='$Fechai' and cja.fecha <='$Fechaf' AND ot.orden=cja.orden and ($Sucursal)
				   ORDER BY cja.fecha,ot.orden";
		...
		$cSql="SELECT cja.id,cja.fecha as fechas,cja.orden,cja.hora,cja.importe,cja.tpago,ot.importe,ot.status,cja.usuario,ot.institucion,ot.fecha,ot.hora as horaorden
				   FROM cja,ot
		 ...
		 if($_REQUEST[Institucion]=='*'){
	        ...
	        while($registro=mysql_fetch_array($res)) {
				 //$InstO=$registro[institucion];
		              ...
		              $AbnD=mysql_query("SELECT sum(importe) FROM cja WHERE orden='$registro[orden]' and fecha='$Fechai' AND id<>'$registro[0]'",$link);
				 ...
				 if($InstO<>$registro[fechas]){
					 if($Ini==1){
							...
							$Recupera2=$Recupera2+$Recupera;
							 ...
							 $Abonos=0;
							 $Ini=$Ini+1;
							 $InstO=$registro[fechas];
							 if ($Orden1==$registro[orden] or $Fecha<>$registro[fechas]){
								$no=$no+1;
							}else{
								$no=$no+0;
							}
							...
							echo "<th align='right'><font size='1' face='Arial, Helvetica, sans-serif'>".number_format($Efectivo+$Tarjeta+$EfectivoR+$TarjetaR,'2')."</font></th>";
							echo "</tr>";
				 ...
				 //}	   
					...
					if( $status == "Recuperac." ){
		                 ...
		                 if($registro[tpago]=="Cheque"){
							...
							echo "<th align='right'><font size='1' face='Arial, Helvetica, sans-serif'>$ ".number_format($Totalefectivo+$Totaltarjeta+$TotalefectivoR+$TotaltarjetaR,'2')."</font></th>";
							echo "<tr><td colspan='14'><hr noshade></td></tr>";
							echo "</table>";
							...
							echo "<td  bgcolor='#CCCCCC'><div align='right'><strong><font size='1' face='Verdana, Arial, Helvetica, sans-serif'>".number_format($Totgralrecupera,"2")."</font></td>";
							echo "<td  bgcolor='#CCCCCC'><div align='right'><strong><font size='1' face='Verdana, Arial, Helvetica, sans-serif'>".number_format($Totgralingreso+$Totgralrecupera,"2")."</font></td>";
							echo "</tr>";
							echo "</table></td>";
							echo "</tr>";
							echo "<tr> </tr>";
